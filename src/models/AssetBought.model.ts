...
export const createAssetBought = (
  { user, assetId, type }: { user: UserModel; assetId: number; type: ASSET_TYPE },
  opt?: { t?: Transaction },
) => {
...
export const findBoughtAssetBy = ({ assetId, userId, videoId, pictureId }: FindBoughtAssetByParams) => {
  ...
  if (assetId) {
    where[Op.or as unknown as string] = [{ pictureId: assetId }, { videoId: assetId }];
  }
  if (userId) {
    where.userId = userId;
  }
