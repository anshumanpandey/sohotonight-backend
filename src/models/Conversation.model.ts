...
export const getConversationBy = async ({
  ...
  id?: string;
  createdByUserId?: string;
  implicatedUserId?: string | number;
  ...
  const c = await ConversationModel.findAll({
    where: by,
    include: [
      { model: UserModel, as: 'toUser', attributes: userAttributes },
      { model: UserModel, as: 'createdByUser', attributes: userAttributes },
      { model: MessageModel, include: [{ model: UserModel }] },
    ],
  });
